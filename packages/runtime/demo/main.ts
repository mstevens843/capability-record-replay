...
function writeManifest(): number {
  ...
  writeJson(join(EVIDENCE, "MANIFEST.json"), {
    note: "Produced by `pnpm demo`. Excludes itself and `redaction-canary/`, which are written after it. Every entry names what produced it; nothing here came from a model.",
...
function bundleReadme(
  ...
  return [
    ...
    "",
    `Result of the run that produced this bundle: **${report.clean ? "CLEAN" : "FAILED"}** — ${report.filesScanned} files, ${report.bytesScanned} bytes, ${report.needles} distinct needles, ${report.hits.length} hits, ${report.forbidden.length} credential-shaped strings, self-test ${report.selfTest.ok ? "passed" : "FAILED"} (${report.selfTest.found}/${report.selfTest.planted}).`,
    "",
