...
export function bundleReadme(input: CoreBundleInput): string {
  ...
  return [
    ...
    "| `canary/` | all five redaction passes, and what each searched for. |",
    "",
    ...
    "",
    "Member names and balances are the other class of value, and they are not the same problem. The",
    "goal never mentioned them, so they were never bound to a parameter, so there is nothing for",
    "substitution to substitute — `src/synthesis/prose.ts` argues that at length. They appear in the",
    "transcript (the model was shown the screen) and in the replay result (the caller asked for",
    "them), and in exactly two other places they must never reach: `synthesized/`, which the first",
    "pass greps for them, and any file describing this run rather than recording it — this README,",
    "`provenance.json` and `spend.json` — which the **fifth** pass greps for them. Both gate.",
    "",
    "The fifth pass takes its scope as the complement of the other four rather than as a list, so a",
    "file added to this directory later is scanned by default and exempting one means writing down",
    "why in `KNOWN_UNSCANNED`. `canary/` is the one entry on that list: those files are written",
    "after the passes run, out of their own results, and what they quote is the recording the fourth",
    "pass covers.",
    "",
  ].join("\n");
}
