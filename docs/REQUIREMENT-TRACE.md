...
| Main demo | `env -u ... pnpm demo` -> 278 files, seven PASS lines, whole-bundle canary CLEAN |
...
| Handoff | `pnpm -F @crr/runtime exec tsx demo/handoff.ts` -> 2 scenarios, canary clean |
| Multi-tenant overlay | `pnpm -F @crr/runtime exec tsx demo/multi-tenant-overlay.ts` -> 3 scenarios, canary clean |
...
| Redaction / canary | `packages/runtime/src/evidence.ts`, `packages/runtime/src/canary.ts`, `packages/discovery/tools/canaries.ts` | `packages/runtime/test/canary.test.ts`, `packages/discovery/test/canary-scopes.test.ts` | `evidence/redaction-canary/`, canary fields in supplemental manifests | `rg -n '\b10041\b|\b250\.00\b|\b50001\b' evidence` and `rg -n '\b10043\b' evidence --glob '!evidence/discovery-live/**' --glob '!evidence/outcome-promotion/**'` exit 1 when clean | Live discovery recording intentionally contains the argument shown to the model; the canary documents its scopes. |
...
| Human escalation and handoff | `packages/runtime/src/intervention.ts`, `packages/runtime/src/resume.ts`, `packages/runtime/src/escalation.ts` | `packages/runtime/test/escalation.test.ts` | `evidence/handoff/` | `pnpm -F @crr/runtime exec tsx demo/handoff.ts` | Operator UI is minimal and polling, not production co-browsing. |
| Multi-tenant reuse | `packages/core/src/overlay.ts`, `packages/runtime/src/overlay-link.ts` | `packages/runtime/test/browser-overlay.test.ts`, `packages/conformance/test/heterogeneity.test.ts` | `evidence/multi-tenant-overlay/` | `pnpm -F @crr/runtime exec tsx demo/multi-tenant-overlay.ts` | One artifact across two browser tenants is proved; one contract across browser and terminal is not. |
