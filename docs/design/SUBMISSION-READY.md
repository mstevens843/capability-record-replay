...
> three times with credentials unset; each run exited 0 with the then-current bundle count and a
> clean whole-bundle canary. That was before the dedicated handoff and multi-tenant overlay evidence
> directories existed, so it is historical, not the current bundle count.
>
...
`packages/runtime/test/demo-contract.test.ts` (9 → 12). That intermediate board is superseded now.
Nothing was removed to get there; every number above is a fresh forced run.

**The same discipline was used for the 2026-08-29 outcome-promotion move.** Sixty-three tests were
added and none removed. Fifty-six of them were the two new files that pass owed:
...
## 2. `pnpm demo` - historical three-run note plus current count

```
historical run 1: exit=0  whole-bundle canary CLEAN   DEMO OK
historical run 2: exit=0  whole-bundle canary CLEAN   DEMO OK
historical run 3: exit=0  whole-bundle canary CLEAN   DEMO OK
...
CLAUDE_CODE_OAUTH_TOKEN pnpm demo`. Byte totals were not re-quoted on 2026-08-30; the current file
count and canary verdict are the measured submission signals. The older run list predates the
dedicated handoff and multi-tenant overlay exhibits; 278 is the current final reviewer-script count.
...
- **The target application is the author's own fixture.** The single largest threat to every
  reliability number in the repository, and no scenario count fixes it. A 0.0% flake rate over a
